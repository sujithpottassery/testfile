    ...
    if (errorCounter > 0) {
       ...
       // $("html, body").animate({scrollTop: $(error).offset().top}, 1000);
        ...
        $.ajax({
            ...
            method: 'post',
            dataType: 'json',
            data: { name_of_user: $("#sname1").val(), user_phone: $("#phone1").val(), user_email: $("#email1").val(), qualification: $("#qua1").val(),course: $("#course1").val(), location: $("#state1").val(), domain: domain, source: '10' },
            success: function (res) {
                emailId = $("#email1").val();
                if (res.code == 200) {
                   ...
                   $(error + " .message").html( msgg);
                    ...
                    window.location.replace('https://www.iaemumbai.co.in/sm/success.html')
...
function submit_lead_form_two(error, form, required, email, phone) {
    ...
    if (errorCounter > 0) {
        ...
        $.ajax({
            ...
            method: 'post',
            dataType: 'json',
            data: { name_of_user: $("#sname2").val(), user_phone: $("#phone2").val(), user_email: $("#email2").val(), qualification: $("#qua2").val(),course: $("#course2").val(), location: $("#state2").val(), domain: domain, source: '10' },
            success: function (res) {
                emailId = $("#email2").val();
                if (res.code == 200) {
                   /* $(error).removeClass("alert-danger alert-warning").addClass("alert-info").fadeIn();
...
function submit_lead_form_three(error, form, required, email, phone) {
    ...
    if (errorCounter > 0) {
        ...
        $.ajax({
            ...
            method: 'post',
            dataType: 'json',
            data: { name_of_user: $("#sname3").val(), user_phone: $("#phone3").val(), user_email: $("#email3").val(), qualification: $("#qua3").val(),course: $("#course3").val(), location: $("#state3").val(), domain: domain, source: '10' },
            success: function (res) {
                emailId = $("#email3").val();
                if (res.code == 200) {
                   /* $(error).removeClass("alert-danger alert-warning").addClass("alert-info").fadeIn();
